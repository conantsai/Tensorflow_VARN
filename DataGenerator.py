...
class DataGenerator:
	...
	def next_batch(self):
		...
		for index in self.video_indices[self.index:self.index+self.batch_size]:
			...
			dirname = line[0]
			audio = line[0] + ".npy"
			audio = np.resize(np.load(audio), (128, 128))
			label = line[0][-5:-4]
			score = line[0][-3:]
		...
		return np.array(data).astype(np.float32), np.array(audios).astype(np.float32), np.array(labels).astype(np.int64), np.array(scores).astype(np.int64), self.indexlist
