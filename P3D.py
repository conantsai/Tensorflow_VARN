...
def inference_varn(_X, _Y, _dropout, BATCH_SIZE):
    ### video net ####################################################################################################
    with tf.variable_scope("video_layer") as scope:
        cnt = 0
        conv1_custom = tf.nn.conv3d(_X, get_conv_weight('firstconv1', [1, 7, 7, RGB_CHANNEL, 64]), strides=[1, 1, 2, 2, 1], padding='SAME')
        conv1_custom_bn = tf.layers.batch_normalization(conv1_custom, training=IS_TRAIN)
        conv1_custom_bn_relu = tf.nn.relu(conv1_custom_bn)
    ...
    ### reason net ###################################################################################################
    reason_layer = tf.layers.dense(inputs=fusion_layer, units=512)
    reason_layer = tf.nn.dropout(reason_layer, keep_prob=0.5)
    reason_layer = tf.layers.dense(inputs=reason_layer, units=256)
    reason_layer = tf.nn.dropout(reason_layer, keep_prob=0.5)
    reason_layer = tf.layers.dense(inputs=reason_layer, units=NUM_CLASS)
    ### reason net ###################################################################################################

    ### predict net ##################################################################################################
    predict_layer = tf.layers.dense(fusion_layer, 512)
    predict_layer = tf.nn.dropout(reason_layer, keep_prob=0.5)
    predict_layer = tf.layers.dense(predict_layer, 256)
    predict_layer = tf.nn.dropout(reason_layer, keep_prob=0.5)
    predict_layer = tf.layers.dense(predict_layer, 1)
    ### predict net ##################################################################################################
